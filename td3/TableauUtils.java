...
public class TableauUtils {
	...
	public static int[][] generationAleatoire(int n, int m, int min, int max) {
		...
		/*
		 * for (int i=0 ; i<elements.length; i++){ for (int j=0 ; j<elements[i].length;
		 * j++){ elements[i][j]= ran.nextInt(max+1-min)+min; } }
		 */
		for (int i = 0; i < elements.length; i++) {
...
=======
	...
	/**
	 * Cette methode permet d'ajouter un element à la fin du tableau @param tab
	 * 
	 * @return int[]
	 */
	...
	public static int[] supprimer(int[] tableau, int index) {
		...
		class Dims {
			 ...
			 * int a = condi?valOk:;
			 * 
			 * int a; if(condi){ a= valOk; }else { if(condi2){ a= valOk2; }else { a =
			 * valElse } }
			 */
